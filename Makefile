...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=tabi \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(TABI_BINARY) \
...
build-docker:
	...
	$(DOCKER) create --name tabi -t -i ${DOCKER_IMAGE}:latest tabi
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp tabi:/usr/bin/tabid ./build/
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(TABI_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/tabi:Z tabi/node "./tabid testnet init-files --v 4 -o /tabi --keyring-backend=test --starting-ip-address 192.167.10.2 --chain-id tabi_9789-1"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\tabid:/tabi\Z tabi/node "./tabid tendermint unsafe-reset-all --home=/tabi"
	@docker run --rm -v $(CURDIR)\build\node1\tabid:/tabi\Z tabi/node "./tabid tendermint unsafe-reset-all --home=/tabi"
	@docker run --rm -v $(CURDIR)\build\node2\tabid:/tabi\Z tabi/node "./tabid tendermint unsafe-reset-all --home=/tabi"
	@docker run --rm -v $(CURDIR)\build\node3\tabid:/tabi\Z tabi/node "./tabid tendermint unsafe-reset-all --home=/tabi"
else
	@docker run --rm -v $(CURDIR)/build/node0/tabid:/tabi:Z tabi/node "./tabid tendermint unsafe-reset-all --home=/tabi"
	@docker run --rm -v $(CURDIR)/build/node1/tabid:/tabi:Z tabi/node "./tabid tendermint unsafe-reset-all --home=/tabi"
	@docker run --rm -v $(CURDIR)/build/node2/tabid:/tabi:Z tabi/node "./tabid tendermint unsafe-reset-all --home=/tabi"
	@docker run --rm -v $(CURDIR)/build/node3/tabid:/tabi:Z tabi/node "./tabid tendermint unsafe-reset-all --home=/tabi"
endif
