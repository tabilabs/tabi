...
// NewCoordinator initializes Coordinator with N EVM TestChain's (Tabi apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if tabiChain, ok := chain.App.(*app.Tabi); ok {
		bondDenom = tabiChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
