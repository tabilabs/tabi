...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `tabi1` and the original bech32 format
	...
	// a sender's own account on Tabi (sender == recipient)
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		packetTransfer := &transfertypes.MsgTransfer{
			...
			Sender:           recipient.String(),        // recipient is the address in the Tabi chain
...
// the IBC denomination, i.e port and channel on Tabi for the voucher. It
// returns an error if:
