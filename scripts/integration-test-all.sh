...
## default port prefixes for tabid
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/tabid keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/tabid init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/tabid add-genesis-account \
    "$("$PWD"/build/tabid keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000atabi,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/tabid gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/tabid collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/tabid validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/tabid start --pruning=nothing --rpc.unsafe \
    ...
    TABI_PID=$!
    echo "started tabi node, pid=$TABI_PID"
    # add PID to array
    arr+=("$TABI_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test tabi node $HOST_RPC ..."
...
stop_func() {
    TABI_PID=$i
    echo "shutting down node, pid=$TABI_PID ..."
